...
function PostDetail() {
  ...
  const loadPost = () => {
    setLoading(true);
    const posts = JSON.parse(localStorage.getItem("wedding_posts") || "[]");
    const found = posts.find(p => p.id === parseInt(id));
    
    if (found) {
      // Update view count
      const updated = { ...found, views: (found.views || 0) + 1 };
      const updatedPosts = posts.map(p => p.id === found.id ? updated : p);
      localStorage.setItem("wedding_posts", JSON.stringify(updatedPosts));
      setPost(updated);
  ...
  if (!post) {
    return (
      <div style={styles.errorContainer}>
        ...
        <p>The wedding story you're looking for doesn't exist.</p>
        <Link to="/posts">
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        <span style={{ ...styles.categoryBadge, background: categories[post.category]?.color || "#666" }}>
      ...
      <div style={styles.imageContainer}>
        <img
          src={post.image || "https://via.placeholder.com/800x400?text=Wedding+Story"}
          alt={post.title}
      ...
      <div style={styles.contentContainer}>
        ...
        <div style={styles.shareSection}>
          ...
          <div style={styles.shareButtons}>
            <button onClick={() => window.open(`https://www.facebook.com/sharer/sharer.php?u=${window.location.href}`)} style={styles.shareBtn}>📘 Facebook</button>
            <button onClick={() => window.open(`https://twitter.com/intent/tweet?text=${post.title}&url=${window.location.href}`)} style={styles.shareBtn}>🐦 Twitter</button>
            <button onClick={() => window.open(`https://wa.me/?text=${post.title} - ${window.location.href}`)} style={styles.shareBtn}>💬 WhatsApp</button>
          </div>
        </div>
      </div>
          ...
          <div style={styles.relatedGrid}>
            {relatedPosts.map(related => (
              <Link key={related.id} to={`/post/${related.id}`} style={styles.relatedCard}>
                <img
                  src={related.image || "https://via.placeholder.com/300x180?text=Wedding"}
                  alt={related.title}
                ...
                <div style={styles.relatedContent}>
                  ...
                  <p style={styles.relatedDate}>{related.date}</p>
                </div>
              </Link>
            ))}
          </div>
...
const styles = {
  ...
  shareBtn: { padding: "8px 20px", background: "#f0f0f0", border: "none", borderRadius: "25px", cursor: "pointer", fontSize: "14px" },
